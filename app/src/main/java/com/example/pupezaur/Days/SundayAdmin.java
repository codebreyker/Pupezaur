...
public class SundayAdmin extends AppCompatActivity implements TimePickerDialog.OnTimeSetListener {
    FirebaseAuth auth;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        FirebaseDatabase.getInstance().getReference("Schedule").child(firebaseUser.getPhoneNumber()).child("Sunday").keepSynced(true);
        ...
        v.setOnTouchListener(new OnSwipeTouchListener(SundayAdmin.this){
            @Override
            public void onSwipeRight() {
                super.onSwipeRight();
                startActivity(new Intent(SundayAdmin.this, SaturdayAdmin.class));
...
//        buton adaugat cursuri
        btn_add.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if ((startTimeString.isEmpty() || endTimeString.isEmpty()) && !scheduleList.isEmpty()) {
                    Toast.makeText(SundayAdmin.this, "Please select start hour", Toast.LENGTH_SHORT).show();
                } else {
        ...
        //        butoanele cu zile
        btn_sunday.setBackgroundResource(R.drawable.btns_days_blue);
        btn_sunday.setTextColor(getResources().getColor(R.color.Black));
        btn_monday.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                startActivity(new Intent(SundayAdmin.this, MondayAdmin.class));
                overridePendingTransition(R.anim.slide_in_left, R.anim.slide_out_right);
                finish();
            }
        });
        btn_tuesday.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                startActivity(new Intent(SundayAdmin.this, TuesdayAdmin.class));
                overridePendingTransition(R.anim.slide_in_left, R.anim.slide_out_right);
                finish();
            }
        });
        btn_wednesday.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                startActivity(new Intent(SundayAdmin.this, WednesdayAdmin.class));
                overridePendingTransition(R.anim.slide_in_left, R.anim.slide_out_right);
                finish();
            }
        });
        btn_thursday.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                startActivity(new Intent(SundayAdmin.this, ThursdayAdmin.class));
                overridePendingTransition(R.anim.slide_in_left, R.anim.slide_out_right);
                finish();
            }
        });
        btn_friday.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                startActivity(new Intent(SundayAdmin.this, FridayAdmin.class));
                overridePendingTransition(R.anim.slide_in_left, R.anim.slide_out_right);
                finish();
            }
        });
...
//                Log.e(TAG, "onDataChange: "+ AllMethods.name );
        ...
        databaseReference = database.getReference("Schedule").child(firebaseUser.getPhoneNumber()).child("Sunday");
        databaseReference.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                for (int i=0; i<snapshot.getChildrenCount(); i++ ) {
                        ...
                        public boolean onLongClick(View view) {
                            for (int i = 0; i < scheduleList.size(); i++) {
                                if (view.getId() == i) {
                                    ...
                                    AlertDialog.Builder builder = new AlertDialog.Builder(SundayAdmin.this);
                                    ...
                                    builder.setItems(options, new DialogInterface.OnClickListener() {
                                        ...
                                        public void onClick(DialogInterface dialogInterface, int j) {
                                            if (j == 0) {
                                                databaseReference = FirebaseDatabase.getInstance().getReference("Schedule").child(firebaseUser.getPhoneNumber()).child("Sunday");
                                                databaseReference.child(scheduleList.get(finalI).getId()).removeValue().addOnCompleteListener(new OnCompleteListener<Void>() {
                                                    ...
                                                    public void onComplete(@NonNull Task<Void> task) {
                                                        if (task.isSuccessful()) {
                                                            Toast.makeText(SundayAdmin.this, isDeleted, Toast.LENGTH_SHORT).show();
                                                        } else {
                                                            Toast.makeText(SundayAdmin.this, notDeleted, Toast.LENGTH_SHORT).show();
                                                        }
                                                    }
                                                });
                                                return;
                    ...
                    if (currentTime.compareTo(endTimeString) < 0 && currentTime.compareTo(startTimeString) > 0 && dayOfWeek == Calendar.SUNDAY) {
...
//            buton chat
        if (id == R.id.chatButton) {
            Intent intent = new Intent(SundayAdmin.this, AdminChatActivity.class);
...
//            buton setari
        if (id == R.id.btn_settings){
            Intent intent = new Intent (SundayAdmin.this, SettingsActivity.class);
            startActivity(intent);
            return false;
        }
//            buton logout
        if (id == R.id.btn_logout) {
            ...
            startActivity(new Intent (SundayAdmin.this, AdminPhoneRegister.class));
            finish();
        }
    ...
    public void onClickDelete (View view){
        for (int i=0; i < scheduleList.size(); i++){
            if (view.getId() == i){
                System.out.println(scheduleList.get(i).getId());
                databaseReference = FirebaseDatabase.getInstance().getReference("Schedule").child(firebaseUser.getPhoneNumber()).child("Sunday");
                databaseReference.child(scheduleList.get(i).getId()).removeValue().addOnCompleteListener(new OnCompleteListener<Void>() {
                    @Override
                    public void onComplete(@NonNull Task<Void> task) {
                        if (task.isSuccessful()) {
                            Toast.makeText(SundayAdmin.this, "Course deleted", Toast.LENGTH_SHORT).show();
                        } else {
                            Toast.makeText(SundayAdmin.this, "Course could not be deleted", Toast.LENGTH_SHORT).show();
                        }
                    }
                });
                return;
